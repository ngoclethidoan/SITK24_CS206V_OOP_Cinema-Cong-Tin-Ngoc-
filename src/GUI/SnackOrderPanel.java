...
public class SnackOrderPanel extends JPanel {
    ...
    private JPanel buildTopBar() {
        ...
        String hint = isBookingMode() ? "  Step 2: choose snacks or skip"
    ...
    private JPanel buildSection(String sectionTitle, boolean isCorn) {
        ...
        for (Item item : ItemDatabase.getAll()) {
            ...
            if (match) {
                ...
                if (isCorn) cornEntries.add(entry); else bevEntries.add(entry);
                grid.add(buildCard(entry));
    ...
    private JPanel buildCard(ItemEntry entry) {
        ...
        JLabel priceLbl = new JLabel(String.format("%,.0f %s",
            entry.item.getPrice(), LanguageManager.t("currency")));
        priceLbl.setForeground(new Color(200, 200, 200));

        // ── quantity controls ──────────────────────────────────────
        JLabel qtyLbl = new JLabel("0", SwingConstants.CENTER);
        qtyLbl.setForeground(Color.WHITE);
        qtyLbl.setFont(new Font("Dialog", Font.BOLD, 14));
        qtyLbl.setPreferredSize(new Dimension(36, 24)); // fixed width prevents layout jumps
        ...
        plus.addActionListener(e -> {
            ...
            card.revalidate();   // ← force layout refresh
            card.repaint();      // ← force visual refresh
            refreshTotal();
        });
        ...
        JPanel right = new JPanel(new FlowLayout(FlowLayout.CENTER, 6, 0));
        right.setOpaque(false);
        right.add(minus); right.add(qtyLbl); right.add(plus);

        JPanel left = new JPanel(new GridLayout(2, 1));
        left.setOpaque(false);
        left.add(nameLbl); left.add(priceLbl);
    ...
    private JPanel buildBottomBar() {
        ...
        confirm.setBackground(ACCENT); confirm.setForeground(Color.WHITE);
    ...
    private void handleSkip() {
        if (isBookingMode()) {
            ...
            JOptionPane.showMessageDialog(this,
                "Seats added to cart! ✅", "Added to Cart", JOptionPane.INFORMATION_MESSAGE);
            mainFrame.showCart();
        }
    }
    ...
    private void handleConfirm() {
        ...
        if (selected.isEmpty()) { handleSkip(); return; }
        ...
        if (isBookingMode()) {
            ...
            mainFrame.getCurrentUser().addSnackToCart(new SnackCartItem(selected));
